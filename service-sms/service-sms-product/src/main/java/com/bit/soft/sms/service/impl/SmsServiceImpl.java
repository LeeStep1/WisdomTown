...
public class SmsServiceImpl extends BaseService implements SmsService {
    ...
    public BaseVo sendSmsMessage(SmsRequest smsRequest) {
        ...
        SmsAccountTemplateEnum smsEnum=  SmsAccountTemplateEnum.getSmsAccountTemplateEnum(smsRequest.getSmsTemplateId());
        try {
            if (smsRequest.getPhoneNumbers().length == 1) {
                //单条发送

                smsSenderComponent.sendSmsSingle(smsEnum, smsRequest.getPhoneNumbers()[0], smsRequest.getParams());
            } else if (smsRequest.getPhoneNumbers().length > 1) {
                //多条发送
                smsSenderComponent.sendMultiSender(smsEnum, smsRequest.getPhoneNumbers(), smsRequest.getParams());
            ...
            CompletableFuture.runAsync(()->{
               ...
               /* smsSendLog.setSendTime(DateUtil.date2String(new Date(),DateUtil.DatePattern.YYYYMMDDHHmmss.getValue()));*/
                smsSendLog.setSmsType(smsEnum.getSmsTemplateType());
    ...
    public BaseVo sendCheckSms(CheckSmsRequest smsRequest) {
        ...
        try {
            ...
            if (total < Integer.valueOf(smsSendLimit)) {
                ...
                cacheUtil.set(smsSendTemplateKey, authCode, EACH_SMS_INTERVAL);
    ...
    public BaseVo checkSmsCode(SmsCodeRequest request) {
        ...
        if(!BeanUtil.isEmpty(vl)){
            ...
            if(request.getSmsCode().equals(codeKey)){
                vo=  new BaseVo(ResultCode.SMS_CHECKED_SUC);
              //  vo.setMsg("验正码正确");
            }else {
                vo=  new BaseVo(ResultCode.SMS_CHECKED_FAIL);
              /*  vo.setCode(ResultCode.WRONG.getCode());
                vo.setMsg("验正码错误");*/
            }

        }else{
            vo=  new BaseVo(ResultCode.SMS_CHECKED_NO_EFFECT);
            /*vo.setCode(ResultCode.WRONG.getCode());
            vo.setMsg("验正码错误");*/
